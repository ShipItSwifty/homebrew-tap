...
2. Edit `Formula/shipit.rb` — change exactly two fields and remove any stale `bottle do` block from the previous release:
...
6. Run the `Bottle` workflow manually for the new version after the formula update lands on `main`, or let the `Update Formula` workflow dispatch it.
...
- The `Update Formula` workflow can poll upstream releases and update the source formula automatically.
